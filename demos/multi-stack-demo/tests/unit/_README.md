...
from src.multi_stack_demo.lambda.api.users.handler import lambda_handler, process_event
...
class TestUsersHandler:
    ...
    def test_process_event_with_filters(self):
        ...
        with patch('src.multi_stack_demo.lambda.api.users.handler.get_dynamodb_client') as mock_dynamodb:
            mock_client = MagicMock()
            mock_dynamodb.return_value = mock_client
...
from src.multi_stack_demo.lambda.shared.utils import setup_logging, get_env_var
...
npm run test tests/unit/ -- --cov=src/multi_stack_demo/constructs
