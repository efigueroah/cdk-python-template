...
synth_task = project.add_task(
    ...
    steps=[TaskStep.exec("cdk synth --context environment=${ENV:-dev}")],
)
...
destroy_task = project.add_task(
    ...
    steps=[TaskStep.exec("cdk destroy --context environment=${ENV:-dev} --force")],
)
...
diff_task = project.add_task(
    ...
    steps=[TaskStep.exec("cdk diff --context environment=${ENV:-dev}")],
)

# Tareas de documentación
docs_build_task = project.add_task(
    "docs:build",
    description="Build documentation with Sphinx",
    steps=[TaskStep.exec("sphinx-build -b html docs docs/_build/html")],
)

docs_clean_task = project.add_task(
    "docs:clean",
    description="Clean documentation build",
    steps=[TaskStep.exec("rm -rf docs/_build/")],
)

docs_open_task = project.add_task(
    "docs:open",
    description="Open documentation in browser",
    steps=[TaskStep.exec("python -m webbrowser docs/_build/html/index.html")],
)

# Tarea para generar diagrama de arquitectura
diagram_task = project.add_task(
    "diagram",
    description="Generate architecture diagram",
    steps=[TaskStep.exec("cdk-dia --target docs/architecture.png")],
)

# Tarea para análisis de seguridad con CDK Nag
security_task = project.add_task(
    "security",
    description="Run security analysis with CDK Nag",
    steps=[
        TaskStep.exec(
            "python -c \"from cdk_nag import AwsSolutionsChecks; print('CDK Nag configured - run synth to see security checks')\""
        )
    ],
)

# Tarea para formateo de código
format_task = project.add_task(
    "format",
    description="Format code with black and isort",
    steps=[TaskStep.exec("black src/ tests/"), TaskStep.exec("isort src/ tests/")],
)
...
precommit_task = project.add_task(
    "pre-commit",
    description="Run pre-commit hooks",
    steps=[TaskStep.exec("pre-commit run --all-files")],
)
